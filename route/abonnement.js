...
app.get('/abonnement', (req, res)=>{
    connection.query('SELECT * FROM abonnement;', [], (err, results)=>{  //[] requetes préparées
      if (err) {
        console.error(err);
      } else {
        res.json(results);
...
app.get('/abonnement/:id', (req, res)=>{
    ...
    connection.query('SELECT * FROM abonnement WHERE id_abonnement = ?;', [id_abonnement], (err, results)=>{  //[] requetes préparées
      if (err) {
        console.error(err);
      } else {
        res.json(results);
...
app.put('/abonnement/:id', (req, res) => {
    ...
    connection.query(
      ...
      [titre, tout_club, toutes_salles, tout_pays, reduc_complement, reduc_medecine, reduc_coach, tarif, id_abonnement],
      (error, data) => {
        if (error) {
          console.error(error);
          res.status(500).send('Erreur du serveur');
  ...
  app.delete('/abonnement/:id', (req, res)=>{
    const id_abonnement = req.params.id;
    connection.query('DELETE FROM abonnement WHERE id_abonnement = ?', [id_abonnement], (err, results)=>{  //[] requetes préparées
      if (err) {
        console.error(err);
      } else {
